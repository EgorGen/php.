...
class FileUserPersist implements UserPersistInterface
{
    ...
    public function get(string $login): ?User
    {
        ...
        foreach($rowUsers as $item) {
            if($item['login'] === $login) {
                return new User(strtolower($item['login']), $item['password'], $item['password'], $item['address'], $item['email'], $item['number'], DateTimeImmutable::createFromFormat('d.m.Y H:i.s', $item['createdAt']));
            }
        }
